...
A Docker-based dev environment that ships JDK 26 (Temurin), `mvnd` (Maven Daemon), a Rust toolchain (rustup-managed), `bun`, `pnpm`, `git`, `jj` (jujutsu), and the Claude Code and Codex CLIs in a single image. The image is meant to be run against an arbitrary host project directory: the host's source tree is bind-mounted at its original host path inside the container (so jj/git worktree metadata resolves). Maven uses Resolver's split local-repository layout: the host's `~/.m2/repository` is a shared RW remote cache mounted at the container's `remote-cache/` prefix, while locally installed artifacts stay in the ephemeral container layer. Extra host paths can be exposed via `--mount` flags or a `$XDG_CONFIG_HOME/maudebox/config.toml` config; any of them can opt into mode `overlay`, which layers a per-worktree writable upper over a read-only host lower.
...
Build the image (defaults: `mvnd 1.0.6`, `jj 0.43.0`, `bun 1.3.14`, `pnpm 11.12.0`, `claude 2.1.207`, `codex 0.144.1`, `playwright 1.61.1`, `@playwright/mcp 0.0.78`, tag `maudebox`):

```
cargo xtask image
cargo xtask image --mvnd-version 1.0.6 --jj-version 0.43.0 --bun-version 1.3.14 --pnpm-version 11.12.0 --claude-version 2.1.207 --codex-version 0.144.1 --playwright-version 1.61.1 --playwright-mcp-version 0.0.78 --tag maudebox
```
...
maudebox rm /path/to/proj         # tear down an instance (workspace if maudebox created it, plus project volumes + state dir)
```
...
Every launch bind-mounts the host repository RW at `/root/.m2/repository/remote-cache`; `/root/.m2/repository/local-installs` remains in the container's writable layer and disappears with `docker run --rm`. The Dockerfile enables Resolver's enhanced split repository with `remotePrefix=remote-cache` and `localPrefix=local-installs`: `MAVEN_OPTS` covers conventional Maven/Maven Wrapper, while `/usr/local/bin/mvnd` is a launcher that prepends the same values as Maven user properties to every request before delegating to `/opt/mvnd/bin/mvnd` (`mvn` points to the launcher too). Request properties survive project-level `.mvn/mvnd.properties` and `.mvn/jvm.config`; daemon JVM properties do not, because a project's `mvnd.jvmArgs` can replace them wholesale. `aether.syncContext.named.basedir.locksDir` points at `remote-cache/.locks`, mapping every container and host Maven to the same conventional host lock directory.

There is no Maven volume to label, list, chown, or remove. An explicit overlay encompassing `/root/.m2/repository` is rejected because it would collide with the built-in bind.

This design intentionally exposes the host artifact cache RW. Resolver routes normal installs away from it, but any container process can still mutate the cache directly; settings and credentials elsewhere in `~/.m2` remain unmounted.

### Opt-in three-layer cache via overlayfs (one or more)

The non-obvious piece is in `entrypoint.sh`. For each mount spec that uses mode `overlay` (e.g. `~/.gradle:~/.gradle:overlay` in `$XDG_CONFIG_HOME/maudebox/config.toml` or via `--mount`), the wrapper:
...
Multi-overlay provides per-worktree isolation for caches such as `~/.cargo`, `~/.gradle`, and `~/.npm` — one overlay per polyglot tool. Mounting overlayfs from inside a container requires `--cap-add SYS_ADMIN` and `--security-opt apparmor=unconfined`, which `maudebox` supplies only when at least one overlay was requested. If a lowerdir is empty or absent, the entrypoint logs a notice and skips that overlay rather than failing. Without any overlay specs, the container receives neither privilege, `MAUDEBOX_OVERLAYS` is unset, and the entrypoint's overlay loop is a no-op.

`maudebox list` aggregates by project (one row per project/instance pair, with a Maven-volume indicator and an `OVERLAYS` count). `maudebox rm <id-or-name-or-path>` removes every volume labelled with that project plus the per-instance state dir, and — if the state dir holds a manifest left by `maudebox new` — also tears down the jj workspace / git worktree. For a path handed to `maudebox <path>` directly, the worktree is left alone since maudebox didn't create it; only the volumes and state dir come off.
...
### Persistent state (Claude + Codex + gh + ssh)

`maudebox` shares a single named volume `maudebox-state` across all containers/worktrees. The volume hosts four isolated subtrees, each mounted at the canonical path the tool expects via Docker's `volume-subpath`:

- `claude/` → `/root/.claude` (Claude login, plugin caches)
- `codex/`  → `/root/.codex` (Codex login, `config.toml`, sessions, SQLite state DB — the image sets `CODEX_HOME=/root/.codex`)
- `gh/`     → `/root/.config/gh` (gh auth, config)
- `ssh/`    → `/root/.ssh` (SSH keys, `known_hosts`)

The four trees never mix on disk despite living in the same volume. `volume-subpath` mounts fail if the subdir doesn't exist, so the wrapper runs a throwaway `mkdir -p /v/claude /v/codex /v/gh /v/ssh` container before each launch (idempotent, ~50ms). Requires Docker 25+ (subpath mounts).

The `codex/` subtree is `$CODEX_HOME`, holding everything Codex persists — including a SQLite state DB it opens in WAL mode with a 5s busy_timeout. Sharing one DB across concurrent containers is safe here for the same reason it is across concurrent sessions on a normal host: WAL needs all writers on one host against a filesystem with working `fcntl` locking + mmap, and a Docker **named volume** lives in the single Docker/OrbStack VM's own ext4 (bind mounts of host paths use virtiofs; named volumes do not), so every container hits the DB as processes on one VM kernel against one real local fs. Keep `CODEX_HOME` on this named-volume subpath — pointing it at an overlay upperdir or a host bind mount would put the DB on a filesystem where WAL genuinely breaks. Unlike Claude's `~/.claude.json`, Codex keeps its login token inside `$CODEX_HOME`, so no symlink redirect is needed. On Linux the entrypoint `chown`s the `codex/` subtree to `HOST_UID` alongside `claude/`/`gh/`/`ssh/`.

On top of the `codex/` subtree, Codex's host-side global config is bind-mounted read-only (only when it exists): `~/.codex/AGENTS.md` → `/root/.codex/AGENTS.md` and `~/.agents/skills` → `/root/.agents/skills`. `AGENTS.md` is Codex's `CLAUDE.md`-equivalent *and* its memory analog — Codex has no separate auto-memory store, so there is no `--memory-from` counterpart. Project-level `AGENTS.md` needs no handling: it already lives inside the bind-mounted worktree.
...
Managed MCP servers (the `[mcp.NAME]` tables in `config.toml`) are layered on top via separate bind mounts, not via the state volume — and one set of tables drives **both** agents. For Claude, the wrapper serializes the tables to `$XDG_STATE_HOME/maudebox/managed-mcp.json` (atomic write) and bind-mounts it read-only at `/etc/claude-code/managed-mcp.json`, which Claude Code reads as enterprise-managed scope; that bind is only added when at least one `[mcp.*]` table is configured. For Codex, the wrapper translates the same tables into `[mcp_servers.NAME]` in `$XDG_STATE_HOME/maudebox/codex-config.toml` and bind-mounts it at `/etc/codex/config.toml` (Codex's lowest-precedence system layer). Both mount-target parent dirs are created in the Dockerfile so they always exist. Atomic rename matters for both: Docker captures the file's inode at mount time, so a later rewrite (from a concurrent maudebox launch with a different config) doesn't perturb an already-running container.

Claude and Codex spell MCP config differently, so the Codex side is a real translation, not a pass-through. Codex uses `[mcp_servers.NAME]` with no `type` field (transport inferred from `command` vs `url`), `http_headers` instead of `headers`, and `additionalProperties: false` (unknown fields are rejected). `translate_mcp_for_codex` maps the shared fields (`command`/`args`/`env`/`url`/`cwd`, and `headers` → `http_headers`), drops `type`, and warns-and-drops anything else. A per-server `[mcp.NAME.codex]` sub-table is the escape hatch for Codex-only keys (`bearer_token_env_var`, `startup_timeout_sec`, …): its contents are passed through verbatim and merged last (so they override the translated fields), and the reserved `codex` key is stripped from Claude's JSON so it doesn't leak in as an unknown field. Unlike the Claude bind, the Codex `config.toml` is written on **every** launch even with no `[mcp.*]` tables, because it also carries the sandbox default (`sandbox_mode = "danger-full-access"`, `approval_policy = "never"`) — the container is already the sandbox, so Codex's own Landlock sandbox and approval prompts are bypassed, mirroring `IS_SANDBOX=1` for Claude. Being the lowest config layer, a user's own `~/.codex/config.toml` (in the state volume) still overrides it.

### Container labels and the `keep` flag for ephemeral instances

Every container is named after its instance (the same string `$MAUDEBOX_INSTANCE` carries inside, set via `docker run --name`) so `docker ps` / `docker exec` / `docker logs` refer to it by a stable, human handle instead of a random `clever_curie`. The container's **hostname** is set to the same handle too (via `--hostname`, sanitized to a valid RFC 1123 label by `sanitize_hostname`): Codex's remote-control identifies a paired server by hostname with no config override, so without this each worktree would enroll in the ChatGPT device list under Docker's random container-ID hostname. This is the Codex analog of what `$MAUDEBOX_INSTANCE` does for `claude --remote-control`. The wrapper still doesn't depend on the name for lookup — that's what labels are for. Each container is stamped with `maudebox.*` labels at run time so the wrapper can find its own containers without parsing image names: `maudebox.instance` (project basename plus any `--instance` discriminator, same as the container name), `maudebox.project` (host path), `maudebox.ephemeral=true|false`. For ephemeral runs (`maudebox new …` without `--keep`), two more labels are set: `maudebox.ephemeral-name` (the user-supplied name) and `maudebox.state-dir` (host path of the per-instance state dir). A name collision (two concurrent maudebox containers on the same project without `--instance`) surfaces as a `docker run` error — the same situation `--instance` already exists to resolve for `$MAUDEBOX_INSTANCE`.

The Docker hostname is set to that same instance handle. Codex's fixed managed-daemon directory, `/root/.codex/app-server-control`, is backed by a separate named volume keyed to the project path and instance handle. This isolates its Unix socket and pid state without splitting the rest of `$CODEX_HOME`, so remote-control daemons can run concurrently while Codex auth/config/history remain shared.
...
`maudebox` derives overlay volume names from a human-readable project key plus a target hash. The basename keeps them recognizable; the hash prevents collisions when two worktrees share a basename. `maudebox rm` removes only that one project's volumes.
...
Codex is installed as the **installer-managed "standalone" layout**, not a bare binary — this is load-bearing for remote control. `codex remote-control start` (the daemon the ChatGPT app and a local `codex --remote` TUI attach to) refuses to run unless it finds the standalone install at `$CODEX_HOME/packages/standalone/current/codex`, because it starts and updates the app-server from that fixed path; a plain binary on `PATH` fails with "managed standalone Codex install not found". So instead of the `codex-<triple>` single-binary asset, the Dockerfile pulls the release **package** bundle (`codex-package-<triple>.tar.gz`) — which additionally vendors the `bwrap` (bubblewrap) sandbox helper and `rg` that Codex expects — and reproduces the installer's release layout: `packages/standalone/releases/<ver>-<target>/` (the extracted bundle plus a `codex → bin/codex` symlink) with `current` → that release. The release tag is still `rust-v<ver>`, and `CODEX_VERSION` remains a Dockerfile `ARG` defaulted in xtask (`--codex-version`).

The non-obvious part is *where* it lives. `$CODEX_HOME` is `/root/.codex`, a maudebox-state volume subpath that's **empty at runtime** (the mount shadows anything baked at build time), yet the daemon needs the standalone install to be under `$CODEX_HOME`. So the layout is staged under **`/opt/codex`** (outside the volume, always present, `chmod a+rX`), and the entrypoint symlinks the volume-backed `$CODEX_HOME/packages/standalone/current` → `/opt/codex/packages/standalone/current` on first launch — a symlink, not a ~336 MB copy into the volume (the `codex` binary alone is ~285 MB). The daemon accepts a `current` that resolves to an external release dir. `releases/` is left as a real dir in the volume so a daemon-side update writes there rather than failing against read-only `/opt`; to keep that from silently un-pinning the version, the wrapper sets `check_for_update_on_startup = false` in the generated `/etc/codex/config.toml`. Pointing `current` at `/opt`'s own (version-agnostic) `current` symlink means an image rebuild with a new `CODEX_VERSION` is picked up with no volume change. The interactive `codex` on `PATH` (`/usr/local/bin/codex`) is a symlink to the same standalone binary. `ENV CODEX_HOME=/root/.codex` is set and the dir pre-created (Codex hard-errors if `CODEX_HOME` points at a missing path).
